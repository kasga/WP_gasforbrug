(function($){
    ...
    window.main = {
        ...
        handleNewsletter: function () {
            ...
            $form.on('submit', function (e) {
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        // Uncheck newsletter
                        $checkbox.closest('.form-group').addClass('hidden');
            ...
            $checkbox.on('change', function () {
                ...
                if ($(this).is(':checked')) {
                    ...
                    // Track nyhedsbrev signup
                    main.tracking('Step 4: Nyhedsbrev');
        ...
        handleResultPage: function () {
            ...
            // Get average
            var average = (parseInt(data.estateSize) * parseFloat(data.estateAge)) + (parseInt(data.residents) * 75);
            var average_price = (average * 7.09) + 525;
            var usage_price = (data.usage * 7.09) + 525;

            // Handle unit = kr.
            if (data.unit === 'kr') {
                usage_price = data.usage;
                data.usage = (data.usage - 525) / 7.09;
            }
            ...
            var result_data = {
                ...
                estateAge: $('[name="estateAge"]').children(':selected').text(),
                residents: parseInt(data.residents, 10),
                gasburnerAge: $('[name="gasburnerAge"]').children(':selected').text(),
                serviceAgreement: (parseInt(data.serviceAgreement, 10) === 1) ? 'Ja' : 'Nej',
            ...
            var help_text = '';
            if (percent < -12) {
                help_text = 'Dit naturgasforbrug ligger {percent} under gennemsnittet. Det er virkelig flot. Du er tydeligvis meget opmærksom på dit naturgasforbrug. Måske kan du alligevel spare endnu mere?';
            } else if (percent <= 0) {
                help_text = 'Dit naturgasforbrug ligger {percent} under gennemsnittet. Godt gået. Du er tydeligvis allerede opmærksom på dit naturgasforbrug. Måske kan du spare endnu mere?';
            } else if (percent > 0 && percent < 21) {
                help_text = 'Dit naturgasforbrug ligger {percent} over gennemsnittet. Det betyder, at der sandsynligvis er penge at spare, hvis du bliver lidt mere opmærksom på dit natrugasforbrug og følger nogle gode spareråd.';
            } else if (percent > 21) {
                help_text = 'Dit naturgasforbrug ligger {percent} over gennemsnittet. Det betyder, at der temmelig sikkert er penge at spare, hvis du bliver opmærksom på dit naturgasforbrug og følger nogle gode spareråd.';
            }

            // Not found?
            if (data.unit == '0' || data.usage == 0) {
                help_text = 'Vi kan ikke fortælle dig, om dit naturgasforbrug ligger under eller over gennemsnittet, men vi kan give dig nogle gode råd til, hvordan du kan opnå besparelser på din varmeregning.';
            }
            ...
            var result_text;
            if (percent < -26) {
                result_text = '<em>Wauu! Du ligger langt under gennemsnitsforbruget</em>.<br>Du ved tydeligvis en hel masse om, hvor du kan spare. Tjek vores spareråd nedenfor – er der endnu mere at komme efter?';

            } else if (percent >= -25 && percent <= -1) {
                result_text = '<em>Flot! Du ligger under gennemsnitsforbruget</em>.<br>Se vores spareråd nedenfor – kan du spare endnu mere, hvis du justerer lidt på nogle vaner?';

            } else if (percent == 0) {
                result_text = '<em>Du bruger det samme som gennemsnittet</em>.<br>Se vores spareråd nedenfor – kan du komme under gennemsnitsforbruget?';

            } else if (percent > 0) {
                result_text = '<em>Du kan spare cirka {saving} kr. om året</em>,<br>hvis du sænker dit naturgasforbrug til gennemsnitsforbruget. Se vores spareråd nedenfor og få tips til, hvordan du sænker dit forbrug.'.replace('{saving}', this.formatNumber(Math.max(0, saving)));
            }
            ...
            TweenLite.to($meter.find('.needle'), 3, {
                rotation: Math.min(105, Math.max(-105, (percent == 0) ? percent : (percent > 0) ? percent + 3 : percent - 3)),
            ...
            TweenLite.to($bar_usage, 3, {
                height: Math.round(parseInt(data.usage)/(interval*($labels.length-1))*100) + '%',
                color: '#fff',
                delay: 0.5,
                ease: Strong.easeOut,
                onUpdate: function () {
            ...
            TweenLite.to($bar_average, 3, {
                height: Math.round(parseInt(average)/(interval*($labels.length-1))*100) + '%',
                delay: 0.5,
                ease: Strong.easeOut,
                onUpdate: function () {
        ...
        handleNavigation: function () {
            // Handle next buttons
            $('[data-action="next"]').hammer().on('tap', function(e) {
                // Get page index
            ...
            // Handle prev buttons
            $('[data-action="prev"]').hammer().on('tap', function(e){
                // Get page index
                var index = $('.page').index($(this).closest('.page'));
            ...
            $('[data-action="first"]').hammer().on('tap', function(e) {
                // Get page index
            ...
            // Handle prev buttons
            $('.progress-bar').children().hammer().on('tap', function(){
                // Get page index
        ...
        changePage: function (index){
            ...
            switch (this.page_index) {
                // Track inital page
                case 0: this.tracking('Step 1: Start'); break;
                case 1: this.tracking('Step 2: Oplysninger'); break;
                case 2: this.tracking('Step 3: Resultat'); break;
            }
                    ...
                    content: {
                        text: function () {
                            if ($(this).next('.tooltip-text').length) {
                                return $(this).next('.tooltip-text').html();
                            } else {
